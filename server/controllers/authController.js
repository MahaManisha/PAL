...
// Google Token Auth (for client-side Google Sign-In via @react-oauth/google)
    ...
    try {
        ...
        if (user) {
            ...
            return jwt.sign(payload, process.env.JWT_SECRET || 'secret', { expiresIn: '1h' }, (err, token) => {
                if (err) throw err;
                res.json({ token, user: { name: user.name, email: user.email, id: user.id, avatar: user.avatar } });
            });
        }
